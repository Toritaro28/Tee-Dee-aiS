...
// --- Linked List Implementation (using new/delete) ---
...
// --- Merge Sort and Binary Search Helper Functions ---
...
public:
    ...
    void savePlaylists();
    void loadPlaylists();
    void clearPlaylists(); // <<-- FIX 1: ADDED METHOD DECLARATION
...
// <<-- FIX 1: ADDED METHOD DEFINITION
void User::clearPlaylists() {
    playlistCount = 0;
    strcpy(currentUsername, "");
    // No need to clear the array itself, playlistCount=0 handles it.
}
...
// <<-- FIX 3: ADDED HELPER FUNCTION
// Helper function to validate TNG phone number format (01X-XXXXXXX)
...
// <<-- FIX 2 & 3: MODIFIED THIS ENTIRE FUNCTION
int User::processPayment(const char* username_payer, const char* itemName, const char* itemType, double amount_to_pay, long long itemExpiry) {
    ...
    if (paymentChoice == 1) {
        ...
        // TNG phone number validation loop
        while (true) {
            ...
            if (isValidTngPhoneNumber(phoneNum)) {
                break; // Format is correct, exit loop
            } else {
        ...
        // VISA card number validation loop
        while (true) {
...
void User::createPlaylist(Admin* admin_ptr) {
    ...
    Song newSong; // Define newSong here to be accessible for recommendations later
    newSong.favoriteCount = 0;
                    ...
                    while (1) {
                        ...
                        if (songChoice >= 1 && songChoice <= displayedCount) {
                            ...
                            if (isDuplicate) {
                                ...
                                // ================== BUG FIX HERE ==================
                                newPlaylist.songs[newPlaylist.songCount++] = selectedSong; // Use selectedSong, not newSong
                                // ================================================
...
#include <conio.h> // Make sure this is included at the top of your file for _kbhit and _getch
...
void User::playPlaylist(UserLogin* userLogin) {
    ...
    // --- NEW DYNAMIC PLAYER LOGIC STARTS HERE ---

    int currentSongIndex = 0;
    bool playerActive = true; // Controls the overall player session

    while (playerActive) {
        // --- CHECK SUBSCRIPTION STATUS AT THE START OF EACH SONG ---
        if (userNode->subscription.type == FREE_TIER) {
            if (userNode->subscription.songsListened >= 10) {
                ...
                _getch();
                playerActive = false; // Exit the main player loop
                continue; // Skip to the next iteration of the while loop (which will exit)
            }
        ...
        // --- SETUP FOR THE CURRENT SONG ---
        Song* currentSong = &selectedPlaylist->songs[currentSongIndex];
        player.playSong(currentSong); // This just prints the "Now Playing" line

        // Increment song count for free tier users
        if (userNode->subscription.type == FREE_TIER) {
        ...
        // --- DYNAMIC PLAYBACK & CONTROL LOOP FOR A SINGLE SONG ---
        const int SONG_DURATION_SECONDS = 10;
        const int LOOP_DELAY_MS = 100; // Update progress bar every 100ms
        ...
        // This loop simulates the song playing for 10 seconds
        while (elapsedTime < SONG_DURATION_SECONDS) {
            // 1. Check for user input without stopping the loop
            if (_kbhit()) {
            ...
            if (skipToNext || skipToPrev || !playerActive) {
                break; // Exit the single-song loop
            }

            // 2. Update and draw the progress bar
            ...
            // 3. Display time and pause status
            ...
            // 4. Update elapsed time if not paused
            if (!isPaused) {
                elapsedTime += (double)LOOP_DELAY_MS / 1000.0;
            }

            // 5. Wait for a short moment
            Sleep(LOOP_DELAY_MS);
        } // End of single-song playback loop

        cout << "\n"; // Move to the next line after the progress bar is finished

        // --- DETERMINE NEXT ACTION AFTER SONG FINISHES OR IS SKIPPED ---
        if (!playerActive) {
            cout << "\n\tExiting player..." << endl;
            Sleep(700);
            break; // Exit the main player loop
        } else if (skipToNext) {
            cout << "\n\tSkipping to next song..." << endl;
            currentSongIndex = (currentSongIndex + 1) % selectedPlaylist->songCount;
        } else if (skipToPrev) {
            cout << "\n\tGoing to previous song..." << endl;
            // Modulo trick for wrapping around to the end from the beginning
    ...
    system("cls"); // Clean up the screen after exiting the player
}
                ...
                if (strcmp(decision_input, "yes") == 0) {
                    ...
                    if (userLogin.validateLogin(userLogin.getUsername(), userLogin.getPassword())) {
                        ...
                        while (1) {
                            ...
                            if (choice == 16) {
                                user.savePlaylists();
                                user.clearPlaylists(); // <<-- FIX 1: CALLED THE METHOD ON LOGOUT
                                cout << "\n\tLogging out";
