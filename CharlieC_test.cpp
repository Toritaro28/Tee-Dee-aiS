...
char* custom_strncpy(char* dest, const char* src, size_t n) {
    ...
    if (n > 0) { // Ensure null termination if n > 0. This is crucial if src is longer than n.
        dest[n - 1] = '\0';
    }
    return dest;
}
...
public:
    ...
    ~Admin() {
        ...
        cout << "Admin content data saved." << endl;
    }
    ...
    void loadPurchasableAlbums() {
        ...
        try {
            ...
            char line[512];
            purchasableAlbumCount = 0;
            // No need for currentAlbum, directly add to purchasableAlbums array
            // when album header is read. Song parsing logic updated to add to last album.
    ...
    void mergeSongs(Song arr[], int l, int m, int r, int sortChoice) {
        ...
        Song L[MAX_RECOMMENDED_SONGS], R[MAX_RECOMMENDED_SONGS]; // Temporary arrays
...
User::~User() {
    ...
    cout << "User object destroyed for " << currentUsername << "." << endl;
}
...
void User::sortSongsInPlaylist() {
    ...
    // Use mergeSortSongs (defined in Admin, but can be copied or made global if needed)
    // For simplicity, let's assume `mergeSortSongs` is accessible or redefine locally if strict class separation.
    // Given the previous code, I'll assume it's okay to call it or copy it.
    // To properly use it without Admin object, it should be a global function.
    // I made it part of Admin, so I'll make a local copy/wrapper or make the original global
    // For now, I'll just copy the implementation to ensure it works.
    // A better design would be a separate utility class or global function for merge sort.
    ...
    public:
        void merge(Song arr[], int l, int m, int r, int sortChoice) {
            ...
            Song L[MAX_SONGS], R[MAX_SONGS]; // Temporary arrays, max size of songs in playlist
...
void mergePurchaseRecords(PurchaseReportRecord arr[], int l, int m, int r, int sortChoice) {
    ...
    PurchaseReportRecord L[100], R[100]; // Temporary arrays, max size 100 as per prompt
...
void searchPurchaseRecords(Admin& admin_ref) { // Friend Function 4
    ...
    do {
        ...
        if (scanf("%d", &searchMethodChoice) != 1 || (searchMethodChoice != 1 && searchMethodChoice != 2)) {
